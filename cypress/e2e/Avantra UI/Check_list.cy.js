...
describe("Check list: create, assert, edit, delete", { defaultCommandTimeout: 5000 }, () => {
    ...
    it("Check for updates creation", function () {
        ...
        const dashname = `Ols_checklist${stamp}`
        cy.get('.dashboard-modify__header-input').type(dashname)
        cy.get('.dashboard-modify__add-dashlet').wait(2000).click()
        cy.get('.dashlet-selector-item__title').contains('Check List').siblings('.dashlet-selector-item__button').click()

        cy.get('[formcontrolname="subtitle"]').children('avantra-input-field').type("Autotest")
        cy.get('.dashlet-settings__param span').contains('Check Selector').siblings('div.dashlet-settings__param--content').click()
        cy.get('avantra-dashlet-settings-check-selector .ng-dropdown-panel-items').contains('ols_all_new').click()
        cy.wait(600)
    ...
    it("Check list assertions created", function () {
        ...
        cy.get('.navigation-list-item').contains('a', dashboardName)
            .wait(2000).click()
        cy.get('.mat-card-title').should('have.text', 'Check List')
        cy.get('.cell-type-status-icon-and-name span')
            .contains('copy of ols_auto')
            .siblings('img')
            .should('have.attr', 'src', 'assets/media/icons/checks/check-warning.svg')
        // cy.get('.cell-type-status-icon-and-name span')
        //     .contains('ols_auto')
        //     .siblings('img')
        //     .should('have.attr', 'src', 'assets/media/icons/checks/check-critical.svg')
        cy.get('.cell-type-status-icon-and-name span')
            .contains('AGENTALIVE')
            .siblings('img')
            .should('have.attr', 'src', 'assets/media/icons/checks/check-ok.svg')
        cy.get('.cdk-column-status')
            .contains('AGENTALIVE').parents('.mat-table tr.mat-row').within(() => {
                cy.get('.mat-column-result')
                .should(($span) => {
                    expect($span.text()).to.contain("Agent is known to be active since");
                })
            })
